/**
 * @fileoverview E2E tests for mobile SEO with iPhone emulation
 *
 * Tests that require actual mobile device emulation:
 * - Responsive layout
 * - Touch-friendly elements
 * - Mobile content readability
 * - Mobile performance hints
 *
 * Uses iPhone 12 device emulation via top-level test.use()
 */

import { expect, test, devices } from '@playwright/test';

// Use iPhone 12 for all mobile tests in this file
// This MUST be at top-level, not inside test.describe()
...
test.describe('Mobile Dynamic Routes SEO', () => {
  ...
  test('Permissive: library page should work on mobile', async ({ page, baseURL }) => {
    if (!baseURL?.includes('permissive')) {
      test.skip();
      return;
    }

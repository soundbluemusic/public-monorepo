...
for (const app of apps) {
  test.describe(`${app.name} - Visual Regression`, () => {
    ...
    for (const viewport of viewports) {
      test(`homepage should match snapshot on ${viewport.name}`, async ({ page }) => {
        ...
        await expect(page).toHaveScreenshot(`${app.name}-home-${viewport.name}.png`, {
          fullPage: true,
          animations: 'disabled',
        });
      });
    ...
    // 상호작용 상태 테스트
    test('should show correct hover states', async ({ page }) => {
      await page.goto(app.url);
      ...
      if ((await firstInteractive.count()) > 0) {
        ...
        await expect(page).toHaveScreenshot(`${app.name}-hover-state.png`, {
          animations: 'disabled',
        });
      }
    });
    ...
    // 포커스 상태 테스트
    test('should show correct focus states', async ({ page }) => {
      await page.goto(app.url);
      ...
      await expect(page).toHaveScreenshot(`${app.name}-focus-state.png`, {
        animations: 'disabled',
      });
    });
  });

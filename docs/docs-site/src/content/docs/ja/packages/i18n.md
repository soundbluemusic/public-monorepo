...
このパッケージは、React Router v7のSSGモード用のi18nルーティングを提供します。英語がデフォルト、韓国語は`/ko`プレフィックスでルーティングされます。
...
import { generateI18nRoutes, extractStaticRoutes } from '@soundblue/i18n';
...
export default {
  ...
  async prerender() {
    ...
    // 動的ルート (両ロケール)
    const entryRoutes = generateI18nRoutes(
      entries,
      (entry) => `/entry/${entry.id}`
    );

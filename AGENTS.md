...
Monorepo with 3 SSR apps (Astro 7 + Cloudflare Workers). **SPA mode is forbidden** — all pages must render complete HTML on the server for SEO.
...
- Build output goes to `dist/` (Astro 7 + `@astrojs/cloudflare` adapter).

...
export default function CategoryPage() {
  const { category, libraries } = useLoaderData<LoaderData>();
      ...
      <FadeIn>
        ...
        <div className="mb-8">
          <h1 className="text-2xl sm:text-3xl font-bold text-(--text-primary) mb-3">
            {isKorean ? category.name.ko : category.name.en}
          </h1>
          <p className="text-lg text-(--text-secondary)">
            {isKorean ? category.description.ko : category.description.en}
          </p>
          <div className="mt-4 text-sm text-(--text-tertiary)">
            {isKorean
              ? `${libraries.length}개의 라이브러리`
        ...
        {libraries.length === 0 && (
          <div className="text-center py-12 text-(--text-secondary)">
            {isKorean ? '이 카테고리에 라이브러리가 없습니다.' : 'No libraries in this category.'}
          </div>
        )}
        ...
        <div className="mt-12 pt-8 border-t border-(--border-primary)">
          <h2 className="text-lg font-semibold text-(--text-primary) mb-4">
            {isKorean ? '다른 카테고리 둘러보기' : 'Explore Other Categories'}
          </h2>

      ...
      <DocsLayout>
        ...
        <div class="flex flex-col sm:flex-row gap-4 mb-8">
          <div class="relative flex-1">
            <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5" style={{ color: "var(--text-tertiary)" }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="space-y-8">
          <For each={Object.entries(groupedLibraries())}>
            {([categoryName, libs]) => (
              <section>
                <h2 class="text-lg font-semibold mb-4 pb-2" style={{ color: "var(--text-primary)", "border-bottom": "1px solid var(--border-primary)" }}>
                  {categoryName}
                </h2>
                      ...
                      >
                        <div class="flex items-start justify-between mb-2">
                          <div class="flex items-center gap-2">
                            <h3 class="font-semibold" style={{ color: "var(--text-primary)" }}>{lib.name}</h3>
                            <button
                              type="button"
                              onClick={(e) => { e.stopPropagation(); toggleFavorite(lib.name); }}
                              class="p-1 rounded-full transition-colors opacity-60 hover:opacity-100"
                              title={favoriteIds().has(lib.name) ? (locale() === "ko" ? "즐겨찾기 해제" : "Remove from favorites") : (locale() === "ko" ? "즐겨찾기 추가" : "Add to favorites")}
                            >
                              <svg class="w-4 h-4" fill={favoriteIds().has(lib.name) ? "currentColor" : "none"} stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" style={{ color: favoriteIds().has(lib.name) ? "#ef4444" : "var(--text-tertiary)" }}>
                                <path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" />
                              </svg>
                            </button>
                            <Show when={lib.usedHere}>
                              <span class="text-xs px-2 py-0.5 rounded-full" style={{ "background-color": "rgba(176, 136, 48, 0.15)", color: "var(--color-warning)" }}>
                        ...
                        <div class="flex items-center gap-3 text-xs">
                          ...
                          <a href={lib.github} target="_blank" rel="noopener noreferrer" class="transition-colors" style={{ color: "var(--text-tertiary)" }} onMouseEnter={(e) => e.currentTarget.style.color = "var(--accent-primary)"} onMouseLeave={(e) => e.currentTarget.style.color = "var(--text-tertiary)"}>GitHub</a>
                          <Show when={lib.npm}>
                            <a href={`https://www.npmjs.com/package/${lib.npm}`} target="_blank" rel="noopener noreferrer" class="transition-colors" style={{ color: "var(--text-tertiary)" }} onMouseEnter={(e) => e.currentTarget.style.color = "var(--accent-primary)"} onMouseLeave={(e) => e.currentTarget.style.color = "var(--text-tertiary)"}>npm</a>
                          </Show>
                        </div>
                      </div>
                    )}

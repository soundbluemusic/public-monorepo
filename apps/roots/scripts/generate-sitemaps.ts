...
const SITE_URL = 'https://roots.soundbluemusic.com';
...
// Main function
async function main() {
  console.log('🗺️  Generating sitemaps for Roots...\n');
  ...
  const conceptNamesPath = join(PUBLIC_DIR, 'concept-names.json');
  const conceptNames = JSON.parse(readFileSync(conceptNamesPath, 'utf-8'));
  const conceptIds = Object.keys(conceptNames);

  // Generate concepts sitemap
  const conceptUrls = conceptIds.map((conceptId) =>
    generateUrlEntry(`/concept/${conceptId}`, '0.7', 'monthly'),
  );
  const conceptSitemap = generateSitemap(conceptUrls);
  writeFileSync(join(PUBLIC_DIR, 'sitemap-concepts.xml'), conceptSitemap);
  console.log(`✅ sitemap-concepts.xml (${conceptIds.length} concepts × 2 languages)`);

  // Generate fields sitemap
  const fieldUrls = fields.map((field) => generateUrlEntry(`/field/${field.id}`, '0.8', 'monthly'));
  const fieldSitemap = generateSitemap(fieldUrls);
  writeFileSync(join(PUBLIC_DIR, 'sitemap-fields.xml'), fieldSitemap);
  console.log(`✅ sitemap-fields.xml (${fields.length} fields × 2 languages)`);
  ...
  // Summary
  const totalUrls = STATIC_PAGES.length + conceptIds.length + fields.length;

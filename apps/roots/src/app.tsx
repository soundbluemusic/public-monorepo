...
/**
 * @fileoverview Roots 앱 루트 컴포넌트
 *
 * 수학 문서 앱의 최상위 컴포넌트로, 라우터 설정과 전역 프로바이더를 구성합니다.
...
function LoadingFallback() {
  return (
    <div class="min-h-screen bg-bg-primary">
      {/* Header skeleton */}
      <div class="fixed top-0 left-0 right-0 h-16 bg-bg-elevated border-b border-border-primary z-40">
        ...
        <div class="container max-w-7xl mx-auto px-4 py-8">
          <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
              <For each={[1, 2, 3, 4, 5, 6]}>{() => <div class="skeleton h-48 rounded-xl" />}</For>
            </div>
          </div>
        </div>
      </div>

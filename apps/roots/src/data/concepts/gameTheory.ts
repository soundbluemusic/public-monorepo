...
export const gameTheoryConcepts: MathConcept[] = [
  {
    ...
    content: {
      ko: {
        ...
        formulas: ['∀i: u_i(s*_i, s*_{-i}) ≥ u_i(s_i, s*_{-i})', '최적 반응: BR_i(s_{-i}) = argmax u_i(s_i, s_{-i})'],
      ...
      en: {
        ...
        formulas: ['∀i: u_i(s*_i, s*_{-i}) ≥ u_i(s_i, s*_{-i})', 'Best response: BR_i(s_{-i}) = argmax u_i(s_i, s_{-i})'],
      ...
      ja: {
        ...
        formulas: ['∀i: u_i(s*_i, s*_{-i}) ≥ u_i(s_i, s*_{-i})', '最適反応: BR_i(s_{-i}) = argmax u_i(s_i, s_{-i})'],
  ...
  },
  {
    ...
    content: {
      ko: {
        ...
        formulas: ['보수 행렬: T > R > P > S', '배신 유혹(T) > 협력 보상(R) > 상호 배신(P) > 호구(S)'],
      ...
      en: {
        ...
        formulas: ['Payoff matrix: T > R > P > S', 'Temptation(T) > Reward(R) > Punishment(P) > Sucker(S)'],
      ...
      ja: {
        ...
        formulas: ['利得行列: T > R > P > S', '裏切りの誘惑(T) > 協力の報酬(R) > 相互裏切り(P) > カモ(S)'],
  ...
  },
  {
    ...
    content: {
      ko: {
        ...
        formulas: ['max_x min_y u(x,y) = min_y max_x u(x,y)', '안장점: u(x*, y) ≤ u(x*, y*) ≤ u(x, y*)'],
        examples: ['가위바위보', '체스', '틱택토'],
        applications: ['인공지능', '의사결정', '최적화']
      },
      en: {
        definition: 'In zero-sum games, optimal strategy minimizes maximum loss',
        formulas: ['max_x min_y u(x,y) = min_y max_x u(x,y)', 'Saddle point: u(x*, y) ≤ u(x*, y*) ≤ u(x, y*)'],
      ...
      ja: {
        ...
        formulas: ['max_x min_y u(x,y) = min_y max_x u(x,y)', '鞍点: u(x*, y) ≤ u(x*, y*) ≤ u(x, y*)'],
  ...
  },
  {
    ...
    content: {
      ko: {
        ...
        formulas: ['영국식: 공개 오름차순', '네덜란드식: 공개 내림차순', '밀봉 1가격: 최고가 지불', '밀봉 2가격 (비크리): 차점가 지불'],
      ...
      en: {
        ...
        formulas: ['English: Open ascending', 'Dutch: Open descending', 'First-price sealed', 'Second-price sealed (Vickrey): Pay second highest'],
      ...
      ja: {
        ...
        formulas: ['イングリッシュ: 公開昇順', 'ダッチ: 公開降順', '封印1価格', '封印2価格 (ヴィックリー): 次点価格支払い'],
  ...
  },
  {
    ...
    content: {
      ko: {
        ...
        formulas: ['복제자 동역학: ẋ_i = x_i(f_i - φ)', '진화적 안정 전략(ESS): u(σ*, σ*) > u(σ, σ*) 또는 같으면 u(σ*, σ) > u(σ, σ)'],
      ...
      en: {
        ...
        formulas: ['Replicator dynamics: ẋ_i = x_i(f_i - φ)', 'ESS: u(σ*, σ*) > u(σ, σ*) or if equal, u(σ*, σ) > u(σ, σ)'],
      ...
      ja: {
        ...
        formulas: ['レプリケーター方程式: ẋ_i = x_i(f_i - φ)', 'ESS: u(σ*, σ*) > u(σ, σ*) または等しければ u(σ*, σ) > u(σ, σ)'],
  ...
  },
  {
    ...
    content: {
      ko: {
        ...
        formulas: ['특성함수: v: 2^N → ℝ', '섀플리 값: φ_i(v) = Σ |S|!(n-|S|-1)!/n! · [v(S∪{i}) - v(S)]', '핵: C(v) = {x: Σx_i = v(N), Σ_{i∈S} x_i ≥ v(S)}'],
      ...
      en: {
        ...
        formulas: ['Characteristic function: v: 2^N → ℝ', 'Shapley value: φ_i(v) = Σ |S|!(n-|S|-1)!/n! · [v(S∪{i}) - v(S)]', 'Core: C(v) = {x: Σx_i = v(N), Σ_{i∈S} x_i ≥ v(S)}'],
      ...
      ja: {
        ...
        formulas: ['特性関数: v: 2^N → ℝ', 'シャプレイ値: φ_i(v) = Σ |S|!(n-|S|-1)!/n! · [v(S∪{i}) - v(S)]', 'コア: C(v) = {x: Σx_i = v(N), Σ_{i∈S} x_i ≥ v(S)}'],

...
export const numericalLinearAlgebraConcepts: MathConcept[] = [
  {
    id: 'lu-decomposition',
    name: {
      ko: 'LU 분해',
    ...
    content: {
      ...
      ja: '行列を下三角行列(L)と上三角行列(U)の積に分解する方法です。連立方程式を効率的に解くときに使用されます。',
    },
  ...
  },
  {
    ...
    content: {
      ...
      ja: '行列を直交行列(Q)と上三角行列(R)の積に分解する方法です。最小二乗問題と固有値計算に使用されます。',
    },
  ...
  },
  {
    ...
    content: {
      ...
      ja: '任意の行列を二つの直交行列と対角行列の積に分解する方法です。データ圧縮、次元削減、擬似逆行列計算に重要です。',
    },
  ...
  },
  {
    ...
    content: {
      ...
      ja: '正定値対称行列を下三角行列とその転置行列の積に分解する方法です。LU分解より効率的で数値的に安定です。',
    },
  ...
  },
  {
    ...
    content: {
      ...
      ja: '連立方程式を反復的に近似して解く方法です。ヤコビ、ガウス・ザイデル、SOR法などがあり、大規模疎行列に適しています。',
    },
    latex:
      'x^{(k+1)} = Bx^{(k)} + c, \\quad \\text{Jacobi: } x_i^{(k+1)} = \\frac{1}{a_{ii}}\\left(b_i - \\sum_{j\\neq i} a_{ij}x_j^{(k)}\\right)',
    relations: {
  ...
  },
  {
    ...
    content: {
      ...
      ja: '正定値対称行列の連立方程式を解く効率的な反復法です。二次形式の最小化と同値で、最適化にも広く使用されます。',
    },
    latex:
      '\\min_x \\frac{1}{2}x^TAx - b^Tx, \\quad r_k = b - Ax_k, \\quad p_{k+1} = r_{k+1} + \\beta_k p_k',
    relations: {
  ...
  },
  {
    ...
    content: {
      ...
      ja: '行列の固有値と固有ベクトルを数値的に計算するアルゴリズムです。べき乗法、QRアルゴリズム、ヤコビ法などがあります。',
    },
    latex:
      '\\text{Power: } v_{k+1} = \\frac{Av_k}{\\|Av_k\\|}, \\quad \\text{QR: } A_k = Q_kR_k, A_{k+1} = R_kQ_k',
    relations: {
  ...
  },
  {
    ...
    content: {
      ...
      ja: '入力の小さな変化に対する出力の感度を測る数です。条件数が大きいと問題がill-conditionedで数値的に不安定です。',
    },

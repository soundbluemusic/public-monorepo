...
/**
 * Script to prevent sidebar collapsed state flash on page load.
 * Should be placed in <head> after DARK_MODE_INIT_SCRIPT.
 *
 * Reads sidebarCollapsed from localStorage and applies 'sidebar-collapsed' class immediately.
 */
export const SIDEBAR_COLLAPSE_INIT_SCRIPT = `(function() {
  try {
    var stored = localStorage.getItem('settings-storage');
    if (stored) {
      var parsed = JSON.parse(stored);
      var collapsed = parsed.state?.sidebarCollapsed;
      if (collapsed) {
        document.documentElement.classList.add('sidebar-collapsed');
      }
    }
  } catch (e) {
    console.error('[Sidebar] Failed to load sidebar preference:', e);
  }
})();`;

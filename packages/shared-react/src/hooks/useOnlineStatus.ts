 ...
 * @example
 * ```tsx
 * import { useOnlineStatus } from '@soundblue/shared-react';
 *
 * function App() {
 ...
 *       {isOnline && wasOffline && <SyncingIndicator />}
 *     </div>
 *   );
 * }
 * ```
 *
 * @module @soundblue/shared-react/hooks/useOnlineStatus
 */
 ...
 * @example
 * ```tsx
 * // Basic usage - show offline banner
 * function App() {
 ...
 * @example
 * ```tsx
 * // Show "syncing" indicator after reconnection
 * function DataView() {
 *   const { isOnline, wasOffline } = useOnlineStatus();
 *   const [isSyncing, setIsSyncing] = useState(false);
 *
 *   useEffect(() => {
 *     // When coming back online after being offline, sync data
 ...
 *   return (
 *     <form>
 *       <textarea name="comment" disabled={!isOnline} />
